...
#df.exp <- read.csv2(paste0(pathData,"raw.exp.mGCP-III.csv"))
...
df.que <- raw.que %>% 
  ...
  mutate(sias = siasd1 + siasd2 + siasd3 + siasd4 + siasd5 + siasd6 + siasd7 + siasd8 + siasd9 + siasd10 + siasd11 + siasd12 + siasd13 + siasd14 + siasd15 + siasd16 + siasd17 + siasd18 + siasd19 + siasd20)%>% 
  mutate_at(
    vars(vp, study),
    funs(as.factor)) %>% 
  mutate_at(
...
tmp4.df.jamovi <- df.exp %>%
  filter(correct_response == "correct") %>% 
  mutate(key = ifelse(cue_dir == "cue_right",
                      ifelse(congruency == "congruent",
                             "m_co1cuR","m_co0cuR"),
                      ifelse(congruency == "congruent",
                             "m_co1cuL","m_co0cuL"))) %>%
...
tmp4b.df.jamovi <- df.exp %>%
  filter(correct_response == "correct") %>% 
  mutate(key = ifelse(cue_dir == "cue_right",
                      #Yes: "cue_right"
                      ifelse(cue_num == "cue_1",
                             #Yes: "cue_3"
                             ifelse(congruency == "congruent",
                                    # Yes: "congruent"
                                    "m_co1cu3R",
...
df.jamovi <- bind_cols(df.que[c(1, 65,68,66:67)], tmp1.df.jamovi[-1], tmp2.df.jamovi[-1], tmp3.df.jamovi[-1], tmp4.df.jamovi[-1],tmp4b.df.jamovi[-1], tmp5.df.jamovi[-1], tmp6.df.jamovi[-1], df.que[c(2:34, 70:73, 35:54, 74, 55:62, 63, 64, 69)])
