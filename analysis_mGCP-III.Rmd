...
dscr.exp.congruency.and.cues <- df.exp.l %>% filter(correct_response == "correct") %>% group_by(congruency, cue_num) %>% summarise_at(vars(rt), funs(min(., na.rm = TRUE), max(., na.rm = TRUE), mean(., na.rm = TRUE), sd(., na.rm = TRUE), se=sd(., na.rm = TRUE)/sqrt(n())))
...
p.subjectT <- df.exp.l %>% filter(correct_response == "correct") %>%
...
p.boxplot3cues <- df.exp.l %>% filter(correct_response == "correct", cue_num == "cue_3", cue_dir == "cue_left") %>%
  ggplot(aes(x = congruency, y = rt, color = congruency)) +
  geom_split_violin() +
  geom_boxplot(alpha =.25) +
...
model = aov(rt ~ congruency * cue_num + Error(subject / (congruency*cue_num)), qr=FALSE, data = df.exp.l)
