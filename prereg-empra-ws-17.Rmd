...
We aim for a total sample size of 60 participants and only stop recruiting when we achieve that number of participants. 


## Sample size rationale
<!-- This could include a power analysis or an arbitrary constraint such as time, money, or personnel. -->

!!!With G*Power we aimed for .75 power to detect even a small effect size of .3 at the standard .05 alpha error probability for the predicted interaction between congruency and the number of cues.
...
We manipulated the amount of gaze cues. There are two blocks, either with one gaze cue or with three gaze cues. In the group-block a trial displayed three faces at fixed positions, all three faces gazed at a specific location (right or left), where in 50% of the trials the target appears. All faces were centered around the glabella and had the same distance to the fixation cross.
The individual-block contained only a single central face which gazed at the same location with the same proportion of valid and invalid trials.
For half of the trials the inter stimulus intervall was 200ms and for the other half 500ms.
...
- Familiarity and attractiveness rating for each face on a five point scale (not for confirmatory analyses)
- Demographic questionnaire (not for confirmatory analyses)
...
The reaction time for each condition (1 vs. 3-gaze cues for congruent and incongruent trials) will be combined into means. 

From the AQ-K we calculate the score as described by the manual.
...
We expect a significant main effect for congruency, as well as an significant interaction between congruency and group size. (for details see: [Follow-up analyses](xx Follow-up analyses))
...
It is expected that participants will miss out on some trials (especially with the strict response window), but participants get excluded when they have less then 50% valid trials.
